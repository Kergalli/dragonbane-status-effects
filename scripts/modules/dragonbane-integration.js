...
import {
  DRAGONBANE_ATTRIBUTES,
  DRAGONBANE_CONDITION_ICONS,
  SELECTORS,
} from "../constants.js";
 ...
 */
function createConditionButton(
  attribute,
  name,
  icon,
  isActive,
...
export function updateDragonbaneConditionStates(html) {
  const token = canvas.tokens.controlled[0];
  if (!token || !token.actor || typeof token.actor.hasCondition !== "function")
    return;
  ...
  DRAGONBANE_ATTRIBUTES.forEach((attr) => {
    const attrLower = attr.toLowerCase();
    const conditionButton = $html.find(
      `${SELECTORS.DRAGONBANE_CONDITION}[data-attribute="${attrLower}"]`
    );
